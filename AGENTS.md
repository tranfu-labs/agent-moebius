...
- 运行桌面应用开发态：`pnpm desktop`
  ...
  - 桌面壳只为 runner 注入 `AGENT_MOEBIUS_DATA_ROOT`；`WORKDIR_ROOT` 默认派生自数据根（`<数据根>/workdir`），无需单独注入 workdir。runner child 显式以 GitHub mode 启动，因此不会重复启动 local console server。
...
- 构建并验证 onboarding 高保真原型：`pnpm --filter @agent-moebius/prototypes check`
...
- 本地主 Agent 收尾验收脚本：`pnpm exec tsx scripts/acceptance/local-console-t5.ts --case primary-agent-closeout`
  - 覆盖用户直接点名专业成员后回到团队主 Agent 收尾，并证明正文中的“验收 / 通过 / 不通过”不会写入本地 acceptance fact、integration event 或触发格式诊断；证据写入 `artifacts/acceptance/t5-evidence.json`。
- 测试：`pnpm test`
- 类型检查：`pnpm typecheck`
...
- 本地 Codex thread 只用于同一次未完成执行的局部恢复：普通新消息、成员接力和下一步骤一律 full；正常退出对已收到 `thread.started` 的活动 run 先把 `graceful-shutdown` intent 写入 session JSONL 再 abort，下次启动自动接管；没有 intent 的崩溃/强杀 orphan 仍落成 stuck。主时间线和子会话 Retry 必须携带精确 `sessionId + runId`，改一改重发在新公开消息之外携带原 runId。恢复前必须校验 thread link、role/团队内容、工作空间指纹和本机 rollout；旧 link、缺失或不兼容时安全 full fallback，禁止按 session+role 或最近 thread 猜测。resume 消费结果与 `cached_input_tokens` 仅写诊断事实，不进入普通对话 DTO。
...
- 默认工作根目录（git worktree 根）派生自数据根 `<数据根>/workdir`，可通过 `AGENT_MOEBIUS_WORKDIR_ROOT` 覆盖；启动日志会打印解析后的路径。
