...
- 更新既有规则「MUST 在 CEO 返回修正文本、后置宽容匹配验证通过后，由 runner 在 post 前于最终 GitHub comment body 末尾追加 `<!-- agent-moebius:ceo-corrected -->` metadata」为：runner 追加 `<!-- agent-moebius:ceo-corrected -->` metadata 的行为按 action 分：
...
And dev codex 本轮返回的 `${LAST_RESPONSE}` 仅表示"看过、没问题、收到"没有实质推进动作，末尾带 `<!-- agent-moebius:stage=in-progress -->`
When runner 在 `postComment` 之前调用 CEO guardrail
