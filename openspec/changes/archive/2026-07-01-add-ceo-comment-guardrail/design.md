...
codex returns rawResponse (any agent)
  → if rawResponse 已含 <!-- moebius:ceo-corrected --> → postComment(rawResponse)  // 防循环
  → format-ceo.judge({
  ...
  → verdict.action === "REPLACE"
      ...
      → runner 在 repaired 末尾追加 <!-- moebius:ceo-corrected --> metadata
...
`<!-- moebius:ceo-corrected -->` metadata 的作用：runner 下轮读到该评论时，凭 body 里这条隐藏标记就能识别"这是 CEO 已修正过的评论，不需要再校"，判定不依赖内存中的响应通道来源；runner 重启、跨轮读取、外部编辑等场景都能正确识别。人眼可见的 CEO quote 标注保留（给人看）；metadata 是给机器看（防循环）。
...
在 persona 结尾加更醒目的 stage marker 契约：**每条响应末尾必须**有 `<!-- moebius:stage=<enum> -->`，附 3 个枚举定义、1-2 组正误对照示例。作用同层 1——**降低 CEO 触发频率**。不影响 CEO 层正确性。
...
**CEO 循环调用：** CEO 自己发出的评论 MUST NOT 再走 CEO。识别机制是评论 body 里的 `<!-- moebius:ceo-corrected -->` metadata——runner 读到该 metadata 就跳过 CEO 拦截。这条 metadata 是持久标记（写在评论 body 里），比依赖内存中的响应通道更鲁棒（runner 重启、跨轮读取都能识别）。
