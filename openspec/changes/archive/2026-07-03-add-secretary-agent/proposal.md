...
- 新增受信任 preScript：`src/agent-prescripts/current-repo-workspace.ts`。它只负责把 Codex cwd 固定到 agent-moebius 当前仓库根目录，避免 secretary 像 `dev` 一样进入目标 issue 的业务仓库 worktree。
...
- 对外行为：用户可通过 `@secretary` 发起 CEO 漏判学习流程；secretary 使用独立 role thread，且 Codex cwd 固定为 agent-moebius 仓库根目录。
