...
另有一条与「Agent 只来自团队」有关：绑定团队的运行名单必须以团队定义的主 Agent 为首，且绑定团队不可用时不得拿共享 `agents/` 顶替。前序已登记规格同时保留存量兼容边界：只有没有团队绑定的既有会话才回退共享 `agents/`。
...
5. 收紧团队名单来源：绑定会话只用团队快照并把真实主 Agent 排在首位，团队已删除 / 需修复时显式阻断；未绑定的存量会话继续按已登记契约回退共享 `agents/`。
...
- `packages/console-ui/src/console/composer-context.tsx`、`session-team-menu.tsx`：把团队已删除 / 需要修复的真实健康状态接到只读说明、红色上下文按钮与改选团队恢复入口。
...
- `src/local-console/types.ts`、`store.ts`、`src/sqlite-state.ts`：承载 `system_event_kind`、continuation 与中断类型在 runtime / worker / HTTP 间的共享契约和序列化适配。
- `desktop/src/team-runtime-binding.ts`、`src/local-console/server.ts`：绑定团队换用真实主 Agent 与健康状态，同时保留未绑定存量会话的共享 `agents/` 回退。
- 上述组件的 `*.test.ts(x)` / `*.stories.tsx` 与 `tests/local-console*.test.ts`：同步收敛后的组件契约，并从真实 HTTP / SQLite / 桌面团队装配验证状态换源、恢复与运行中分流；story 改动只用于保持变体与状态枚举示例可编译且不再传播废弃语义。
...
- 已绑定会话的 Agent 只来自该团队；没有团队绑定的既有会话继续使用共享 `agents/`，不会被误判成团队已删除。
