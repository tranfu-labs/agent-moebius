...
| 文件 | 小节 | 变更 | 状态 |
| --- | --- | --- | --- |
| `docs/product/pages/main-conversation.md` | 页面结构 · 子会话卡片 / 区域与信息 · 子会话卡片 / 响应式与窗口行为 | 无行为条文变更；归档时将本 change 的最终线框回流到页面结构 | 已核对并回流 |
...
- `packages/console-ui/src/console/run-block.tsx`：可见的运行命令输出在分栏收窄后横向滚动，不撑宽页面。
- `packages/console-ui/src/index.ts`：导出新增卡片与展开区类型和组件，供桌面真实装配消费。
- `desktop/src/console-page/app.tsx`、`state-sync.ts`：接通子会话聚合/视图读取、展开区刷新与关闭，并保证侧边栏选择回退只落到父会话。
...
- `src/local-console/types.ts`、`store.ts`、`src/sqlite-state.ts`、`sqlite-state-worker.ts`：为聚合读、卡片消息来源、持久化和父会话选择回退补齐类型与 SQLite 命令接线；不改变父子关系 schema。
- `tests/local-console-child-session-summary.test.ts`、`tests/local-console.test.ts`、`tests/local-console-timeline-truth.test.ts`、`tests/local-console-system-event-kind.test.ts`、`desktop/tests/console-state-sync.test.ts` 及上述 console-ui 组件共置测试：从 HTTP、真实 session-aware 名单装配、桌面选择装配、SQLite 重启与组件入口覆盖跨边界行为。
...
保持不变的核心语义：子会话仍由既有编排链路创建，`session_edges` 的持久化语义与 `parent_session_id` 服务运行时编排和恢复的用途不变；本片只在这条链路上增加卡片锚点、聚合读取与会话级团队快照继承验证。
