...
## 背景
Electron 操作台虽然复用了 `@moebius/console-ui` 的 React 组件，但桌面构建只用 esbuild 的 CSS loader 复制了 `globals.css`，没有执行 Tailwind/PostCSS。最终 `app.css` 仍包含 `@tailwind` / `@apply` 指令，Chromium 无法生成组件 utility 样式，页面退化为裸 HTML 与一份不完整的桌面兜底 CSS。

## 提案
- 让 `@moebius/console-ui` 在 package build 阶段生成完整编译的 `dist/style.css`，并通过 `globals.css` package export 暴露该产物。
