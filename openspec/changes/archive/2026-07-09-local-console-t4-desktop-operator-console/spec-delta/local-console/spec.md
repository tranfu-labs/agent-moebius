# local-console spec delta：local-console-t4-desktop-operator-console

T4 将 T2/T3 的本地通道升级为数据正确级桌面操作台客户端。该 delta 只规定一个本地项目下多会话、运行直播、中断、卡住状态和本地错误记录；不规定 T5 的交棒总线、CEO 兜底、子会话编排、验收全功能对等，也不规定 T6 的 GitHub/local 互斥启动 flag。
...
- MUST preserve the T2 default session and compatibility message endpoint, mapping it to the default local session.
- MUST render local session timelines from `session_messages` without inventing GitHub issue concepts.
- MUST keep local console state in the existing `.state/local-console.sqlite`; T4 must not create a second persistence file disconnected from T2/T3.
...
- MUST show a non-empty live summary for every running local session; when structured JSONL cannot be parsed, the UI must fall back to raw tail text or a deterministic "running / waiting for output" summary.
...
### 边界
- MUST NOT modify `conversation`, `triggers`, agent mention parsing, stage parsing, CEO guardrail, goal-ledger business rules, or GitHub issue runner semantics to satisfy T4.
...
- MUST NOT implement T6 GitHub/local mutually exclusive startup flag or cross-mode data migration in this change.
...
- MUST provide code-verified evidence for the three T4 baseline scenarios: live run display, user interruption, and local failure record.
- SHOULD include QA-suggested evidence for bounded tail reads, cross-session interrupt safety, stuck state, and refresh persistence once product-manager or a human user confirms those suggestions as formal acceptance statements.
- MUST store acceptance screenshots or equivalent media under `artifacts/acceptance/` in the issue worktree and reference them in the final `code-verified` response.
- MUST update `docs/roadmap/milestone-4-local-console.md` T4 with acceptance evidence and mark T4 complete before commit.
