...
产品事实源锚点：`docs/product/pages/main-conversation.md`。本次主要为**实现缺口**——PRD 已写清核心行为，实现尚未做到；归档仅按 `wireframes.md` 回流补充常驻禁用原因的位置与零项目状态。

| 文件 | 小节 | 变更 | 状态 |
| --- | --- | --- | --- |
| `docs/product/pages/main-conversation.md` | 页面结构 / 区域与信息 · 页面标题 / 操作与反馈 · 创建一段对话 / 选择项目与添加项目 / 区域与信息 · 输入框 | 归档时回流 `wireframes.md`：补充常驻发送禁用原因的位置与零项目状态；其余条文由本 change 兑现 | 已写入 |
...
- `packages/console-ui/src/console/role-composer.tsx`：新增 `submitDisabled`，并让 Enter 提交同样遵守禁用态。
- `packages/console-ui/src/index.ts`：导出新对话页组件。
...
- `src/local-console/types.ts`、`src/sqlite-state.ts`、`src/local-console/store.ts`、`src/sqlite-state-worker.ts`：沿存储链路透传 `initialMessage`，并在创建会话事务内条件写入首条消息。
