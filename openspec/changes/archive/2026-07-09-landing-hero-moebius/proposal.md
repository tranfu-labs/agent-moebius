# 提案：landing-hero-moebius

## 背景
落地页 `sites/marketeam/index.html` 首屏仍是借来的 Marketeam 营销人才平台壳，而首屏下方正文已如实介绍 moebius，全页**品牌错位**。本 change 把首屏也换成 moebius，让整页一致。

## 提案
改造首屏内容（机制不变，只换内容），使其如实呈现 moebius：

- **品牌**：Logo `Marketeam` → `moebius`；nav `Your Team/Solutions/Blog/Pricing` → `Overview / How it works / Docs / GitHub`（前两项锚到正文①②）；右侧 `Log In/Join Now` → `GitHub / Get started`。
- **主标题**（打字机）：`@mention a role. Your AI team ships it.`
- **按钮/徽章**：`Start Project` → `Get started →`；`David` 光标徽章 → `@ceo` mention 芯片（示意「@角色」交互）。
- **圆环**：中心 `20k+ Specialists`（对 moebius 是假数字）→ **CEO 居中（orchestrates & guards）+ 6 个真实角色绕轨**（secretary / dev / dev-manager / product-manager / qa / hermes-user）；hover 角色出真实 charter（与正文③一致）。移除中心 count-up。
...
- 目录名 `sites/marketeam/` 保持不变（避免连带改动引用），仅内容改为 moebius。
