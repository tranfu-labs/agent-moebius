...
2. 若 trigger 为 `skip/no-trigger`，再检查是否满足 T8 兜底条件：
   ...
   - 最新 timeline message 归一化后 `speaker = "user"`，且原始 comment body 不含任何 `agent-moebius:*` runner 机器 metadata；也就是最新 comment 不是 dead-letter、CEO append、agent envelope 或其他 runner 发布产物。
...
<!-- agent-moebius:ceo-reviewed action=fail_open reason=codex-timeout -->
...
- `conversation.ts` 的 speaker 归一化只依赖 `agent-moebius:role=...`，不把 `ceo-reviewed` 当 role metadata。
...
- `gh issue view 41 --repo tranfu-labs/agent-moebius --json comments`：
  - 2026-07-04T01:37:23Z，product-manager 评论“PM 方案验收结论：暂不通过”，带 `agent-moebius:role=product-manager`，URL `https://github.com/tranfu-labs/agent-moebius/issues/41#issuecomment-4880201671`。
  - 2026-07-04T01:37:42Z，product-manager 评论“方案验收结论：通过”，带 `agent-moebius:role=product-manager`，URL `https://github.com/tranfu-labs/agent-moebius/issues/41#issuecomment-4880202402`。两者相隔 19 秒。
  - 2026-07-04T01:46:16Z，product-manager 评论“PM 验收结论：不通过”，带 `agent-moebius:role=product-manager`，URL `https://github.com/tranfu-labs/agent-moebius/issues/41#issuecomment-4880222742`。
  - 2026-07-04T01:47:00Z，product-manager 评论“代码验收结论：通过”，带 `agent-moebius:role=product-manager`，URL `https://github.com/tranfu-labs/agent-moebius/issues/41#issuecomment-4880224488`。两者相隔 44 秒。
...
- `/tmp`：`find /tmp -maxdepth 1 -type d -name 'agent-moebius-*'` 无输出。
...
- 新 metadata 可能影响历史 parser。缓解：speaker 归一化只解析 `agent-moebius:role=...`；新增测试锁定 `ceo-reviewed` 不改变 speaker。
