...
- MUST 让所有 runner 发布路径的评论 body 带可审计 CEO 覆盖标记，例如 `<!-- agent-moebius:ceo-reviewed action=... -->`；实际调用 CEO 的评论 MUST 标明 CEO 结果，不实际调用 CEO 的系统错误评论 / dead-letter / 路由 append MUST 标明 bypass 或 not-applicable reason。
...
Then comment body MUST 包含 `<!-- agent-moebius:role=dev -->`
And comment body MUST 包含 `<!-- agent-moebius:ceo-reviewed action=no_change -->`
And comment body MUST NOT 包含 `<!-- agent-moebius:ceo-corrected -->`
...
And 追加的 ceo comment MUST 包含 `<!-- agent-moebius:role=ceo -->`
And 追加的 ceo comment MUST 包含 `ceo-reviewed` 标记
And 追加的 ceo comment MUST 继续包含 `<!-- agent-moebius:ceo-corrected -->`

### 场景：未调用 CEO 的系统评论显式标注 bypass 或 not-applicable
Given 媒体准备失败、artifact 发布失败或 dead-letter 发布路径产生可见评论
When runner 发布该评论
Then comment body MUST 包含 `<!-- agent-moebius:ceo-reviewed action=bypass ... -->` 或 `<!-- agent-moebius:ceo-reviewed action=not_applicable ... -->`
