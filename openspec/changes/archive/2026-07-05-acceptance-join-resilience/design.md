...
1. 统计当前 issue 时间线里含 `<!-- agent-moebius:acceptance-format-reminder -->` 标记的评论数。
...
- **user 分支**：现有行为完全不变（issue-body 目标形状 digest key、`hasAgentMoebiusMetadata` 守卫、active 门、comment-id 防重）。
- **agent 分支**：`latestMessage.speaker ∈ agentNames` 且 `source === "comment"` 且 intake mode 为 active 时进入（不套 `hasAgentMoebiusMetadata` 守卫——agent 评论必然带 stage marker 等 metadata）：
