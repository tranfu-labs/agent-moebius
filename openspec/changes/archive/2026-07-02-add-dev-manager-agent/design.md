...
- **方案评估方法论**：讨论 / 评估技术方案时的一组并行判断维度，不分先后：先看现成的（优先搜英文网络最佳实践 / 成熟开源框架 / 项目现有能力，能复用不自造）、可行性与可靠性（失败模式 / 边界 / 降级 / 回滚）、影响面与副作用（module-map 依赖边界 / 既有契约 / 新增 BUG / 回归 / 安全漏洞）、成本与演进（实现与维护成本 / 技术债）。
