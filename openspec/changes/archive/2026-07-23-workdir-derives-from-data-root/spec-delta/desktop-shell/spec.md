...
本 delta 修改 `openspec/specs/desktop-shell/spec.md` 的 `### 数据根` 小节：把原「靠打包态注入 `AGENT_MOEBIUS_WORKDIR_ROOT` 防止 workdir 逃逸」的条目，替换为「`WORKDIR_ROOT` 默认值本身派生自数据根」。合并时以本节整体替换同名 `### 数据根` 小节，NEVER 与旧版并存；其余未列条目保持原样。
