...
- MUST 支持以**对话型 issue runner**形态运行：盯单一指定 issue（首批为 `tranfu-labs/moebius#1`），把 `issue.body + comments` 视作一段对话历史。
...
- MUST 把本地脚本每次执行的 stdout / stderr 落到 `<TMP_ROOT>/moebius-<ISO>-c<count>/` 下，并在日志中打印该路径，便于追溯。
...
Given `tranfu-labs/moebius#1` 当前 `comments.length = 0`（仅 body）
...
And `<TMP_ROOT>/moebius-<ISO>-c1/` 下保留 codex 的 `stdout.jsonl` 与 `stderr.log`
