...
## 域定位
`goal-ledger` 负责把长期 GitHub issue 协作中的目标、里程碑、任务、阶段、质量基准、验收语句、依赖、provenance、父子 issue reference 与 run manifest reference 落成本地可信事实源。它解决“用户目标只能部分接收，后续澄清缺少可恢复账本”的问题。
