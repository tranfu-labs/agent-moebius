...
### 场景 38：CEO guardrail — dev 收到收敛指令后无推进按 ceo.md 识别场景处理
...
And `agents/ceo.md` 的识别场景清单覆盖该情形（持续推进 / 死锁等待）
When runner 调用 CEO guardrail 并传入 `lastReflectorHook = <该 hook body>`
Then CEO 按 `agents/ceo.md` 识别场景处理并返回相应输出
