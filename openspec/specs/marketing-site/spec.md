...
- 工牌架点击/激活 MUST 打开详情并**此时才显示工牌正面**：转场为标准容器变形，且 MUST 保留黑胶唱片架的「拿起」动作——被点击的工牌 MUST 先原地拿起（上抬、前移并由侧立 ≈44° 转正），再作为**共享元素**从举起姿态连续飞抵**弹窗内工牌位**（非面板中心），途中由简化卡面交叉淡化为完整工牌，落定后与真工牌像素对齐换装；转场全程同一时刻 MUST 只存在一份该工牌的可见实例（架上原卡隐位、弹窗真工牌落定才现身），MUST NOT 双卡同屏或让被追踪的卡中途消失；弹窗呈现正面放大工牌 + 角色名、charter、`@<role>` 用法；MUST 键盘可达（focus + Enter）。
