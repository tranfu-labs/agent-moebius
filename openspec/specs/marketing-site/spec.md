...
- 工牌架点击/激活 MUST 打开详情并**此时才显示工牌正面**：被点击的工牌 MUST 以**容器变形转场**（iOS 打开 App 式）慢慢变成详情弹窗——工牌卡面从架上侧立（≈44°）位置一边转正放大一边淡出，弹窗面板从该工牌的屏幕足迹处同步放大淡入，两者共享同一条运动路径；弹窗容器自身 MUST NOT 再做独立的 3D 翻转入场；弹窗呈现正面放大工牌 + 角色名、charter、`@<role>` 用法；MUST 键盘可达（focus + Enter）。
- 关闭（Esc / 遮罩 / 关闭按钮均 MUST 可关）MUST 逆向动画：弹窗面板缩回工牌足迹并淡出、工牌卡面转回侧立落回架上原位，架上原卡恢复、焦点还给触发工牌；弹窗打开期间页面发生滚动后逆向变形 MUST 仍落准架上当前位置。
