...
系统 MUST 在 Codex 发出 `thread.started` 后，通过 session fact 写漏斗为对应 run 追加包含 `runId`、源消息 id、role、threadId、startedAt 与恢复上下文指纹的关联事实；同一 run 的同值重放 MUST 幂等，冲突 threadId 或指纹 MUST fail closed。旧事实缺少指纹时 MUST 可读，但 MUST NOT 直接 resume。系统 MUST NOT 把 Codex rollout 内容复制进 Moebius session JSONL，也不得只在 SQLite 或进程内保存该关联。
