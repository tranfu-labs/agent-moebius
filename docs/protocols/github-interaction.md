...
Runner 写回 agent 评论时会自动生成可见前缀和 metadata：`<role>:` 前缀加 `<!-- moebius:role=... -->`。人工评论和 loop watcher 补发评论必须以自己身份平文发言，不得手写或伪装 role envelope。Metadata 是 speaker 归一化的唯一可靠依据，伪装会污染各 role thread 的对话事实。
