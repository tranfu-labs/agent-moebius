...
打开 sites/marketeam/index.html（agent-moebius 品牌）
  ...
  │     └─ 点击/Enter：弧线容器变形——工牌沿先升（拿起）后滑入的连续弧线飞抵弹窗内工牌位，转正在弧线前段完成、全程水平速度不断流（丝滑契约），途中简化卡面渐变为完整工牌；变形面从足迹长成面板、内容不拉伸，【此时才显正面】= 正面放大工牌 + 角色名/charter/@用法（Esc/遮罩/按钮关=更快逆向弧线：升离弹窗→临近插槽才侧立落回，键盘可达）
