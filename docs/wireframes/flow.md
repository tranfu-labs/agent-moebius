...
打开 sites/marketeam/index.html（agent-moebius 品牌）
  ...
  ├─ 4 轨持续旋转（左30/右40/右50/左60 s），中心 CEO 与绕轨角色反向自转正立（无 count-up）
  ├─ 角色错峰 fly-in（scale0.3+rotate-180+blur → 正常，0.6→2.3s）
  │     └─ hover 角色 → 弹 charter 弹框（角色名·短标签 + charter，复用 tooltip，无复制）
  ├─ 底部 ticker 技术栈 Node.js·TypeScript·Codex·gh·Electron ×4 无缝左滚 20s，两端渐隐
