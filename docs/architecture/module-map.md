...
### github-issue-runner
- 职责边界：常驻运行，轮询 `tranfu-labs/agent-moebius#1`，把 issue body + comments 视作对话历史；当最新可见消息艾特了 `agents/*.md` 中存在的 agent 时触发本机 codex 并回评 GitHub issue。
- 入口：`pnpm start` → `src/runner.ts`
- 上游：进程启动命令、本机 `gh auth login`、本机 `codex` CLI。
- 下游：`src/github.ts`、`src/conversation.ts`、`src/codex.ts`、`agents/*.md`。
