...
import {
  MOEBIUS_DATA_ROOT_ENV,
  MOEBIUS_WORKDIR_ROOT_ENV,
  resolveRuntimePaths,
} from "../src/config.js";

// 正例数据根刻意与 projectRoot（源码根）分叉，避免同值掩盖 workdir 逃逸。
const PROJECT_ROOT = "/opt/app/source";
const DATA_ROOT = "/home/user/.moebius";
...
describe("resolveRuntimePaths workdir 派生", () => {
  it("未设环境变量时 workdir 派生自数据根（此处即 projectRoot），不落在源码目录旁", () => {
    ...
    expect(paths.workdirRoot).not.toBe(path.resolve(PROJECT_ROOT, "..", "moebius-workdir"));
  });

  it("设了数据根时 workdir 跟随数据根，而非源码根", () => {
    const paths = resolveRuntimePaths({
      env: { [MOEBIUS_DATA_ROOT_ENV]: DATA_ROOT },
  ...
  it("不再读取旧命名的数据根环境变量", () => {
    const paths = resolveRuntimePaths({
      env: { [LEGACY_DATA_ROOT_ENV]: "/home/user/legacy-data" },
      projectRoot: PROJECT_ROOT,
    });
  ...
  it("MOEBIUS_WORKDIR_ROOT 作为显式覆盖优先于默认派生", () => {
    const override = "/mnt/fast-disk/moebius-worktrees";
    const paths = resolveRuntimePaths({
      env: {
        [MOEBIUS_DATA_ROOT_ENV]: DATA_ROOT,
        [MOEBIUS_WORKDIR_ROOT_ENV]: override,
      },
      projectRoot: PROJECT_ROOT,
    });
