...
当前官网只有一个静态页面：[`index.html`](./index.html)。CSS 和 JavaScript 均已内联，品牌图标由同目录的 `assets/` 提供；不需要安装依赖、执行构建或启动 Node.js 服务。
...
5. 在浏览器 DevTools 中确认没有未预期的 JavaScript 错误、404 或页面横向滚动；页头 Logo、favicon 和 Apple Touch Icon 都应返回 `200`。
